"""
The Energy balance models response to a periodic forcing Chapter 5.2 Figure 5.7
"""
...
if __name__ == "__main__":
    ...
    #fig.subplots_adjust(right=0.91, top=0.98, left=0.165, hspace=0)
    trip_flag = False
    ...
    ax1.set_ylim(-50, 50)
    ax1.set_yticks(np.linspace(-40, 40, num=5, endpoint=True))
    ax1.yaxis.set_minor_locator(MultipleLocator(10))
    ax1.set_ylabel(r'Energy (W\,$\cdot$\,m$^{-2}$)')
    ax1.grid(True, which='major', linestyle='--', alpha=0.5)
    ax1.grid(True, which='minor', axis='y', linestyle='--', alpha=0.5)
    ...
    twin1.set_yticks(np.linspace(-90, 90, num=5, endpoint=True))
    twin1.set_yticklabels(np.linspace(-90, 90, num=5, endpoint=True).astype(int))
    twin1.set_ylabel(r'Mean Temperature ($^\circ$C)')
    twin1.set_ylim(-90, 90)
    ...
    plt.savefig('./Forcing_Model_result_Increasing_E_130.pdf', format='pdf', dpi=300)