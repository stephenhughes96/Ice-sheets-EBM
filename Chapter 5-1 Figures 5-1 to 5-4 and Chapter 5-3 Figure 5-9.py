"""
This code covers section 5.1 of the thesis and is designed to solve the model for equilibrium.
(change in temperature with time is 0)
"""
...
'''
This Function calculates the weighted average based on the area of each latitudinal band
'''
...
#Import Earth real latitudinal temperatures and solar insolation from file
lats = np.flip(np.arange(0, 180, 2.5)-88.75)
T = np.flip(np.load('./Input_data/Satellite_T_LAT.npy'))
S = np.flip(np.load('./Input_data/S_0BP.npy'))

"""
Figure 5.1: Bringing the model to equilibrium using the CERES parameters obtained in Chapter 4
"""
A = 213.6
B = 1.75
D = 3
...
"""
Figure 5.2: Bringing the model to equilibrium using North 1975 parameters from Table 3.1
"""
A = 211.2
B = 1.55
D = 3.75
...
"""
Figure 5.3: Bringing the model to equilibrium using McGuffies parameters from Table 3.1
"""
A = 204
B = 2.17
D = 3.81
...
"""
Figure 5.4: Bringing the model to equilibrium using the values obtain from a least squares fit as presented in Table 5.3
"""
A = 202.8
B = 2.17
D = 3.81
...
"""
Figure 5.9: Bringing the model to equilibrium for T_Eq, T_D and T_S
"""
A = 202.8
B = 2.17
D = 3.81
...
fig.subplots_adjust(hspace=0.3)
plt.tight_layout()
plt.savefig('./Equilibrium_T_EQ_S_D.pdf', format='pdf', dpi=300) #Figure 5.9 of Thesis